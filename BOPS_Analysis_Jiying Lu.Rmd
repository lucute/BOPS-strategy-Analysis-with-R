...
cas2_7_f <- lm(logamount ~ factorgender + 
    age_band + I(age_band^2) + est_income_code + length_of_residence + 
   factorchild+ factorhome + bops*month + month_index + factorsummary, 
    data = cas2)

plot(effect(term="bops:month", mod=cas2_7_f,xlevels=2),multiline=TRUE)
...
cas6$factorethnic <- as.factor(cas6$est_income_code)

cas6_11_f <- lm(logamount ~ factorgender + 
    age_band + est_income_code + factorethnic + factorchild + bops*month + 
    month_index + factorsummary, data = cas6)

plot(effect(term="bops:month", mod=cas6_11_f,xlevels=2),multiline=TRUE)

#Store5998
summary(cas5998_2)
cas5998$factorsummary <- as.factor(cas5998$summary)

cas5998_2_f <- lm(formula = logamount ~ bops*month + 
    month_index + factorsummary, data = cas5998)

plot(effect(term="bops:month", mod=cas5998_2_f,xlevels=2),multiline=TRUE)
...
cas2_6f <- lm(logamount ~ factorgender + 
    age_band + I(age_band^2) + est_income_code + length_of_residence + 
    factorchild + factorhome + bops * factorsummary + 
    month_index, data = cas2)

plot(effect(term="bops:factorsummary", mod=cas2_6f,xlevels=2),multiline=TRUE)

summary(cas6_10)

cas6_10f <- lm(logamount ~ factorgender + age_band + est_income_code + factorethnic + factorchild + bops * factorsummary + 
    month_index, data = cas6)

plot(effect(term="bops:factorsummary", mod=cas6_10f,xlevels=2),multiline=TRUE)

summary(cas5998_3)
cas5998_3f <- lm(formula = logamount ~ month + bops + 
    month_index + factorsummary*bops, data = cas5998)

plot(effect(term="bops:factorsummary", mod=cas5998_3f,xlevels=2),multiline=TRUE)
